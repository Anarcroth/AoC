...
with open('day3.txt') as f:
    ...
    for compartment in rucksack:
        ...
        first_compartment, second_compartment = compartment[:length//2], compartment[length//2:]
        intersection = set(first_compartment).intersection(set(second_compartment))
        unique = list(intersection)[0]
        if ord(unique) >= 97:
...
with open('day3.txt') as f:
    ...
    for i in range(0, len(rucksack), 3):
        slice_item = slice(i, i + 3, 1)
        groups.append(rucksack[slice_item])

    for group in groups:
        groups = [set(group[0]), set(group[1]), set(group[2])]
