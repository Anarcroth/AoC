...
with open('day1.txt') as f:
    ...
    for c in elf_calories:
        if c == '':
            max_cals_per_elf.append(temp_max)
            temp_max = 0
        else:
            temp_max += int(c)

print(f'Part 1: {max(max_cals_per_elf)}')
...
(with 24000 Calories), then the third Elf (with 11000 Calories), then the fifth
Elf (with 10000 Calories). The sum of the Calories carried by these three elves
is 45000.
...
max_cals_per_elf.sort(reverse=True)
print(f'Part 2: {sum(max_cals_per_elf[0:3])}')
