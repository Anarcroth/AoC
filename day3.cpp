...
void up(int spaces, int& last_y, int& last_x);
void down(int spaces, int& last_y, int& last_x);
void left(int spaces, int& last_y, int& last_x);
void right(int spaces, int& last_y, int& last_x);

void draw_wire(std::map<char, int>& wire, int& last_y, int& last_x);

std::map<char, int> parseInput(std::string input);
...
int main()
{
    ...
    std::map<char, int> wire1 = parseInput(input1);
    std::map<char, int> wire2 = parseInput(input2);
    ...
    int last_x = 1;
    int last_y = 10;
    panel[last_y][last_x] = 'o';
    draw_wire(wire1, last_y, last_x);

    //last_x = 1;
    //last_y = 10;
    //draw_wire(wire2, last_y, last_x);
...
void draw_wire(std::map<char, int>& wire, int& last_y, int& last_x)
{
    for (auto x = wire.rbegin(); x != wire.rend(); ++x)
    {
	if (x->first == 'U')
	{
	    up(x->second, last_y, last_x);
	}
	else if (x->first == 'D')
	{
	    down(x->second, last_y, last_x);
	}
	else if (x->first == 'L')
	{
	    left(x->second, last_y, last_x);
	}
	else if (x->first == 'R')
	{
	    right(x->second, last_y, last_x);
	}
    }
}
...
std::map<char, int> parseInput(std::string input)
{
    ...
    std::map<char, int> wire_seq;
    while ((pos = input.find(delimiter)) != std::string::npos)
    {
	wire_seq.insert(std::pair<char, int>(input.substr(0, 1)[0], std::stoi(input.substr(1, pos))));
